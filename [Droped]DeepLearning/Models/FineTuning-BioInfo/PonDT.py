...
from DeepLearning.Util import (PonDataset4FineTuning, PonMetrics, EarlyStopping, logger_init,
                               ...
                               embedding_dataset_creator, embedding_model_getter)
...
def training(dataset: PonDataset4FineTuning,
    ...
    with tqdm(range(2000)) as progress_bar:
        for epoch in range(2000):
            ...
            train_steps = 0
            model.train()
            for wii, wam, mii, mam, bio, label in trainLoader:
                # 载入GPU
                wii, wam, mii, mam, bio, label = (wii.to(device), wam.to(device), mii.to(device),
                                                  mam.to(device), bio.to(device), label.to(device))
                output = model(wii, wam, mii, mam, bio)  # 预测模型
            ...
            model.eval()
            with torch.no_grad():
                for wii, wam, mii, mam, bio, label in validateLoader:
                    # 载入GPU
                    wii, wam, mii, mam, bio, label = (wii.to(device), wam.to(device), mii.to(device),
                                                      mam.to(device), bio.to(device), label.to(device))
                    output = model(wii, wam, mii, mam, bio)  # 预测模型
...
def testing(dataset: PonDataset4FineTuning,
    ...
    with torch.no_grad():
        with tqdm(range(len(dataset))) as progress_bar:
            for wii, wam, mii, mam, bio, label in testLoader:
                # 载入GPU
                wii, wam, mii, mam, bio, label = (wii.to(device), wam.to(device), mii.to(device),
                                                  mam.to(device), bio.to(device), label.to(device))
                output = model(wii, wam, mii, mam, bio)  # 预测模型
...
if __name__ == "__main__":
    ...
    all_logger.debug(Config.extraction_model_name)
    # endregion
    ...
    train_dataset, test_dataset = embedding_dataset_creator(df=Config.dataset,
