...
cache_path = os.path.join(rel_path, r"pssm/swissprot")
...
def PssmGotFromUrl(df: pd.DataFrame,
    ...
    def variation_getter_from_pssmcool(ID: str) -> np.ndarray:
        ...
        if not os.path.exists(ID_pssm_cache_path):
            warnings.warn(f"Path error in pssmGetter, check this {ID_pssm_cache_path}")
            return np.array([np.nan] * 400)    # Subsequently, np.allclose(result, 0) is used to
            # determine
            # whether
            # there is an error
            # return np.zeros((400, ))    # Subsequently, np.allclose(result, 0) is used to determine  whether there is an error
    ...
    def pssm_getter_from_blast(ID: str,
        ...
        if not os.path.exists(cache_path + f"/pssm/{ID}.pssm"):
            ...
            # 运行psi-blast
            # psiblast -comp_based_stats 1 -db ./pssm/SwissProtDataset/swissprot -query  ./pssm/swissprot/fasta_file.txt -evalue 0.001 -num_iterations 3 -num_threads 12 -out_ascii_pssm ./pssm/swissprot/pssm/Q72SM7.pssm
