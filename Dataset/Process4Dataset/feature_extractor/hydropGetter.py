...
class ScaleGetter:
    ...
    def scale_got_from_url(self,
        ...
        target_types = ["Molecular weight",
                        ...
                        "Hphob. / Black",
                        "Hphob. / Bull & Breese",
                        "Hphob. / Fauchere et al.", "Hphob. / Guy",
                        "Hphob. / Janin",
        ...
        for selected_type in target_types:
            ...
            print(selected_type, "finished...", score)
            ...
            with tqdm.tqdm(total=df.shape[0]) as processingBar:
                for rowIndex, row in df.iterrows():
                    ...
                    if wild_label not in scale_cache:
                        wild_result = self.scale_got_from_url(ID, index, wild_sequence)
                        scale_cache[wild_label] = wild_result
                    else:
                        wild_result = scale_cache[wild_label]
                    # mutant scores
                    if mutant_label not in scale_cache:
                        mutant_result = self.scale_got_from_url(ID, index, mutant_sequence)
                        scale_cache[mutant_label] = mutant_result
                    else:
                        mutant_result = scale_cache[mutant_label]
                    # result
                    result[ID] = wild_result - mutant_result

                    np.save(scale_cache_path, np.array(scale_cache))
                    processingBar.update(1)
        except Exception:
            raise RuntimeError("访问ProtScale出错，请检查网络连接...")
        finally:
            # save the history
            np.save(scale_cache_path, np.array(scale_cache))
